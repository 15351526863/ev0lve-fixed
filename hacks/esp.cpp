...
void esp::draw_player(const gui_legacy::draw_adapter &draw, cs_player_t *const player) const
{
	...
	{
		int health = player->get_health(); // Получаем здоровье игрока
		float current_health = static_cast<float>(health) / 100.0f; // Нормализуем здоровье (0.0f - 1.0f)
		current_health = std::clamp(current_health, 0.0f, 1.0f);  // Ensure value is between 0 and 1

		// Вычисляем цвет полоски здоровья в зависимости от здоровья
		//evo::ren::color col;
		//if (health >= 50) {
		...
		bld.add(esp_side_left,
			esp_item::bar(current_health, player_entry.visuals.previous_health,
				...
				: XOR_STR(""),
				col)); // Передаем вычисленный цвет полоске здоровья
	}
