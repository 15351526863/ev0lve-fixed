							   ...
							   bool &send_packet)
{
	...
	if (!rag.did_stop()  && game->client_state->choked_commands == 0 && !attempted_to_jump)
	{
	...
	if (!rag.did_stop())
	{
		...
		if (local_player->is_on_ground())
		{
			...
			else if ((cfg.rage.slowwalk.get() || aa.is_fakeducking()) && !peek_assistant.has_shot)
				...
				&& cmd->command_number > 1 && pred.info[(cmd->command_number - 1) % input_max].flags & cs_player_t::on_ground)
			{
				...
				const auto amnt = local_player->get_velocity().length2d() / 50.f;
