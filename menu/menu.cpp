...
void menu_manager::create()
{
	...
	ctx->res.general.avatar = std::make_shared<texture>(data::ava, sizeof(data::ava));
	...
	draw.textures[GUI_HASH("gui_avatar")] = ctx->res.general.avatar;
	...
	// user
	ctx->user.username = XOR("deadhuqe");
	...
	const auto window_sz = vec2{ 800.f, 530.f };
