...
const wchar_t* security::internal::get_string(int index) {
	...
	switch (index) {
	case 0: value = xor ("Qt5QWindowIcon").crypt_get(); break;
	case 1: value = xor ("OLLYDBG").crypt_get(); break;
	case 2: value = xor ("SunAwtFrame").crypt_get(); break;
	case 3: value = xor ("ID").crypt_get(); break;
	case 4: value = xor ("ntdll.dll").crypt_get(); break;
	case 5: value = xor ("antidbg").crypt_get(); break;
	case 6: value = xor ("%random_environment_var_name_that_doesnt_exist?[]<>@\\;*!-{}#:/~%").crypt_get(); break;
	case 7: value = xor ("%random_file_name_that_doesnt_exist?[]<>@\\;*!-{}#:/~%").crypt_get(); break;
	}

	static std::wstring wide_value;
	wide_value = std::wstring(value.begin(), value.end());
	return wide_value.c_str();
}
...
int security::internal::memory::check_window_name() {
	...
	for (const wchar_t* name : names) {
		if (FindWindowW(name, 0)) { return security::internal::debug_results::find_window; }
	}
...
int security::internal::memory::nt_query_information_process() {
	...
	NtQueryInformationProcess = (_NtQueryInformationProcess)GetProcAddress(h_ntdll, xor ("NtQueryInformationProcess").crypt_get());
...
int security::internal::memory::debug_active_process() {
	...
	CreateMutexW(NULL, FALSE, get_string(5));
	...
	WCHAR cmdline[MAX_PATH + 1 + sizeof(int)];
	swprintf_s(cmdline, sizeof(cmdline) / sizeof(WCHAR), xor (L"%s %d").crypt_get(), sz_path, pid);
...
int security::internal::memory::write_buffer() {
	...
	if (GlobalGetAtomNameW(INVALID_ATOM, (LPWSTR)buffer, 1) != FALSE || GetEnvironmentVariableW(get_string(6), (LPWSTR)buffer, 4096 * 4096) != FALSE || GetBinaryTypeW(get_string(7), (LPDWORD)buffer) != FALSE
...
int security::internal::exceptions::debug_string() {
	SetLastError(0);
	OutputDebugStringA(xor ("anti-debugging test.").crypt_get());
...
int security::internal::virtualization::check_registry() {
	HKEY h_key = 0;
	if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, xor (L"HARDWARE\\ACPI\\DSDT\\VBOX__").crypt_get(), 0, KEY_READ, &h_key) == ERROR_SUCCESS) { return security::internal::debug_results::check_registry; }

	return security::internal::debug_results::none;
}

int security::internal::virtualization::vm() {
	if (CreateFileW(xor (L"\\\\.\\VBoxMiniRdrDN").crypt_get(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 0, OPEN_EXISTING, 0, 0) != INVALID_HANDLE_VALUE) { return security::internal::debug_results::vm; }

	if (LoadLibraryW(xor (L"VBoxHook.dll").crypt_get())) { return security::internal::debug_results::vm; }

	HKEY h_key = 0;
	if ((ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, xor (L"SOFTWARE\\Oracle\\VirtualBox Guest Additions").crypt_get(), 0, KEY_READ, &h_key)) && h_key) { RegCloseKey(h_key); return security::internal::debug_results::vm; }

	h_key = 0;
	if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, xor (L"HARDWARE\\DESCRIPTION\\System").crypt_get(), 0, KEY_READ, &h_key) == ERROR_SUCCESS)
	{
		unsigned long type = 0;
		unsigned long size = 0x100;
		wchar_t* systembiosversion = (wchar_t*)LocalAlloc(LMEM_ZEROINIT, (size + 10) * sizeof(wchar_t));
		if (ERROR_SUCCESS == RegQueryValueExW(h_key, xor (L"SystemBiosVersion").crypt_get(), 0, &type, (unsigned char*)systembiosversion, &size))
		{
			to_lower((unsigned char*)systembiosversion);
			if (type == REG_SZ || type == REG_MULTI_SZ)
			{
				if (wcsstr(systembiosversion, xor (L"vbox").crypt_get()))
				{
		...
		type = 0;
		size = 0x200;
		wchar_t* videobiosversion = (wchar_t*)LocalAlloc(LMEM_ZEROINIT, (size + 10) * sizeof(wchar_t));
		if (ERROR_SUCCESS == RegQueryValueExW(h_key, xor (L"VideoBiosVersion").crypt_get(), 0, &type, (unsigned char*)videobiosversion, &size))
		{
			if (type == REG_MULTI_SZ)
			{
				wchar_t* video = videobiosversion;
				while (*(unsigned char*)video)
				{
					to_lower((unsigned char*)video);
					if (wcsstr(video, xor (L"oracle").crypt_get()) || wcsstr(video, xor (L"virtualbox").crypt_get())) { return security::internal::debug_results::vm; }
					video = &video[wcslen(video) + 1];
				}
			}
		}
	...
	HANDLE h = CreateFileW(xor (L"\\\\.\\pipe\\VBoxTrayIPC").crypt_get(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
	if (h != INVALID_HANDLE_VALUE) { CloseHandle(h); return security::internal::debug_results::vm; }

	unsigned long pnsize = 0x1000;
	wchar_t* w_provider = (wchar_t*)LocalAlloc(LMEM_ZEROINIT, pnsize * sizeof(wchar_t));

	h_key = 0;
	if ((ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, xor (L"SYSTEM\\CurrentControlSet\\Enum\\IDE").crypt_get(), 0, KEY_READ, &h_key)) && h_key)
	{
		unsigned long n_subkeys = 0;
		unsigned long max_subkey_length = 0;
		if (ERROR_SUCCESS == RegQueryInfoKeyW(h_key, 0, 0, 0, &n_subkeys, &max_subkey_length, 0, 0, 0, 0, 0, 0))
		{
			//n_subkeys is usually 2
			if (n_subkeys)
			{
				wchar_t* w_new_key = (wchar_t*)LocalAlloc(LMEM_ZEROINIT, (max_subkey_length + 1) * sizeof(wchar_t));
				for (unsigned long i = 0; i < n_subkeys; i++)
				{
					memset(w_new_key, 0, (max_subkey_length + 1) * sizeof(wchar_t));
					HKEY h_new_key = 0;

					if (ERROR_SUCCESS == RegEnumKeyW(h_key, i, w_new_key, max_subkey_length + 1))
					{
						if ((RegOpenKeyExW(h_key, w_new_key, 0, KEY_READ, &h_new_key) == ERROR_SUCCESS) && h_new_key)
						{
							unsigned long nn = 0;
							unsigned long maxlen = 0;
							RegQueryInfoKeyW(h_new_key, 0, 0, 0, &nn, &maxlen, 0, 0, 0, 0, 0, 0);
							wchar_t* w_newer_key = (wchar_t*)LocalAlloc(LMEM_ZEROINIT, (maxlen + 1) * sizeof(wchar_t));
							if (RegEnumKeyW(h_new_key, 0, w_newer_key, maxlen + 1) == ERROR_SUCCESS)
							{
								HKEY HKKK = 0;
								if (RegOpenKeyExW(h_new_key, w_newer_key, 0, KEY_READ, &HKKK) == ERROR_SUCCESS)
								{
									unsigned long size = 0xFFF;
									unsigned char value_name[0x1000] = { 0 };
									if (RegQueryValueExW(h_new_key, xor (L"FriendlyName").crypt_get(), 0, 0, value_name, &size) == ERROR_SUCCESS) {
										to_lower(value_name);
										if (strstr((char*)value_name, xor ("vbox").crypt_get())) { return security::internal::debug_results::vm; }
									}
									RegCloseKey(HKKK);
								}
							}
							LocalFree(w_newer_key);
