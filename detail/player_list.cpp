...
namespace detail
{
	...
	void i_player_list::recv_esp_data(uint8_t* data, int from, svc_msg_voicedata_t* msg)
	{
		...
		const auto cbk = [&](lua::state& state)
			{
				std::cout << "Calling on_voice_message callback" << std::endl;

				state.create_table();
				state.set_field(XOR_STR("client"), reinterpret_cast<sdk::cs_player_t*>(game->client_entity_list->get_client_entity(from)));
				state.set_field(XOR_STR("audible_mask"), msg->audible_mask);
				state.set_field(XOR_STR("xuid"), msg->xuid);
				state.set_field(XOR_STR("xuid_low"), inf.xuid_low);
				state.set_field(XOR_STR("xuid_high"), inf.xuid_high);
				state.set_field(XOR_STR("voice_data"), msg->data);
				state.set_field(XOR_STR("proximity"), msg->proximity);
				state.set_field(XOR_STR("caster"), msg->caster);
				state.set_field(XOR_STR("sequence_bytes"), msg->sequence_bytes);
				state.set_field(XOR_STR("section_number"), msg->section_number);
				state.set_field(XOR_STR("uncompressed_sample_offset"), msg->uncompressed_sample_offset);
				state.set_field(XOR_STR("format"), msg->format);

				return 1;
			};

		lua::api.callback(FNV1A("on_voice_message"), cbk);
	...
	void i_player_list::on_update_end(cs_player_t* const player)
	{
		...
		for (auto i = 0; i < player_entry.records.size(); i++)
		{
			...
			{
				const auto num_to_remove = player_entry.records.size() - i;
				for (size_t k = 0; k < num_to_remove; ++k)
				{
					if (player_entry.records.empty())
						break;
